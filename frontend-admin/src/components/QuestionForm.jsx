...
const SHAPES = {
  square: { symbol: '⬛', name: '正方體', code: 'C' },
  triangle: { symbol: '🔺', name: '三角錐', code: 'T' },
  circle: { symbol: '🟢', name: '球體', code: 'S' },
};
...
export default function QuestionForm() {
  ...
  const generateBasicAnswers = () => {
    ...
    Object.entries(shapeCoordinates).forEach(([shape, coords]) => {
      console.log(`${SHAPES[shape].name} (${SHAPES[shape].symbol}): x=${coords.x}, y=${coords.y}`);
    });
